    ...
    for step in range(n_steps):
        ...
        if step % checkpoint_freq == 0:
            ...
            if args.model_save and step >= args.model_save:
                # XXX change checkpoint path here
                ckpt_dir = args.checkpoint_path / "checkpoints"
                ckpt_dir.mkdir(exist_ok=True)

                test_env_str = ",".join(map(str, test_envs))
                filename = "lastcheckpoint_TE{}_{}.pth".format(test_env_str, step)
                ...
                if not args.debug:
                    ...
                    if step % 5000 == 0 and step != 0:
                        torch.save(save_dict, path)
                        logger.info("save last model to %s" % path)
                    else:
                        pass
                else:
                    logger.debug("DEBUG Mode -> no save (org path: %s)" % path)
                # 保存accuracies最高的模型
                if records[-1]["train_out"] > best_acc:
                    best_acc = records[-1]["train_out"]
                    # print(best_acc)
                    best_path = ckpt_dir / "bestcheckpoint_TE{}_{}.pth".format(test_env_str, step)
                    torch.save(save_dict, best_path)
                    logger.info("save best model to %s" % best_path)
