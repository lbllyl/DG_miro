...
class Mixstyle(Algorithm):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        ...
        # self.featurizer = networks.ResNet(input_shape, self.hparams, network) # XXX change, not origin
    ...
    def update(self, x, y, args, **kwargs):
        ...
        _, core_l, _ = torch.svd(logit)
        condition_loss_l = torch.sum(core_l[:int(core_l.shape[0]/2)])/torch.sum(core_l[int(core_l.shape[0]/2):])
        condition_loss_l = 0.04 * condition_loss_l
        # exit(0)
        condition_loss = torch.sum(core[:int(core.shape[0]/2)])/torch.sum(core[int(core.shape[0]/2):])
        # print(condition_loss)
        # exit(0)
        condition_loss = 0.04 * condition_loss
        # print(condition_loss)
        loss = loss + condition_loss
...
class VREx(ERM):
    ...
    def update(self, x, y, args, **kwargs):
        ...
        for i, (x, y) in enumerate(minibatches):
            ...
            # XXX key change
            # _, core, _ = torch.svd(logits)
            _, core, _ = torch.svd(self.classifier.weight)
            torch_condition_number = torch.linalg.cond(self.classifier.weight)
            # print(logit.shape)
            # print(core.shape)
            _, core_l, _ = torch.svd(logits)
            condition_loss_l = torch.sum(core_l[:int(core_l.shape[0]/2)])/torch.sum(core_l[int(core_l.shape[0]/2):])
            condition_loss_l = 0.005 * condition_loss_l
            # exit(0)
            condition_loss = torch.sum(core[:int(core.shape[0]/2)])/torch.sum(core[int(core.shape[0]/2):])
            # print(condition_loss)
            # exit(0)
            condition_loss = 0.1 * condition_loss
            # print(condition_loss)
            # exit(0)
            losses[i] = losses[i] + condition_loss

            losses[i] = losses[i] + condition_loss_l
...
class GroupDRO(ERM):
    ...
    def update(self, x, y, args, **kwargs):
        ...
        for m in range(len(minibatches)):
            ...
            _, core_l, _ = torch.svd(logit)
            condition_loss_l = torch.sum(core_l[:int(core_l.shape[0]/2)])/torch.sum(core_l[int(core_l.shape[0]/2):])
            condition_loss_l = 0.05 * condition_loss_l
            # exit(0)
            condition_loss = torch.sum(core[:int(core.shape[0]/2)])/torch.sum(core[int(core.shape[0]/2):])
            # print(condition_loss)
            # exit(0)
            condition_loss = 0.05 * condition_loss
            # print(condition_loss)
            losses[m] = losses[m] + condition_loss
...
class MLDG(ERM):
    ...
    def update(self, x, y, **kwargs):
        ...
        for (xi, yi), (xj, yj) in random_pairs_of_minibatches(minibatches):
            ...
            # print(condition_loss)
            # exit(0)
            condition_loss = 0.001 * condition_loss
            # print(condition_loss)
            inner_obj = inner_obj + condition_loss
...
class AbstractMMD(ERM):
    ...
    def update(self, x, y, **kwargs):
        ...
        for i in range(nmb):
            ...
            condition_loss = 0.001 * condition_loss
            # print(condition_loss)
            objective = objective + condition_loss
