...
class Evaluator:
    def __init__(
        self, test_envs, eval_meta, n_envs, logger, evalmode="fast", debug=False, target_env=None, eval_transforms=None
    ):
    ...
    def accuracy_from_loader(self, algorithm, loader, weights, debug=False):
        correct = 0
        total = 0
        losssum = 0.0
        weights_offset = 0
        ...
        for i, batch in enumerate(loader):
            ...
            if self.eval_transforms is not None:
                x = self.eval_transforms(batch["x"]).to(device)
            else:
                x = batch["x"].to(device)
    ...
    def accuracy(self, algorithm, loader_kwargs, weights, **kwargs):
        ...
        return self.accuracy_from_loader(algorithm, loader, weights, **kwargs)
    ...
    def evaluate(self, algorithm, ret_losses=False):
        ...
        for name, loader_kwargs, weights in self.eval_meta:
            ...
            acc, loss = self.accuracy(algorithm, loader_kwargs, weights, debug=self.debug)
            accuracies[name] = acc
            losses[name] = loss
